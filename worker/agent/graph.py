...
def safety_check(state: TutorState) -> TutorState:
    ...
    blocked = any(p.search(latest) for p in _PATTERNS)
...
def pedagogical_reasoner(state: TutorState) -> TutorState:
    if state.get("blocked"): response = SAFETY_BLOCK_MESSAGE
        ...
        response = ("Nice — that step keeps the equation balanced. What operation would undo the remaining term attached to x?" if ev and ev.valid else "That step changes the equation's balance. What same operation could you apply to both sides to undo that term?" if ev else "What operation could you undo first on both sides to move closer to isolating x?")
...
def run_graph(messages: list[ChatMessage]) -> TutorState:
    return GRAPH.invoke({"request_id":str(uuid.uuid4()),"messages":messages,"blocked":False,"math_evaluation":None})
